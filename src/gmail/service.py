...
class GmailTool:
    ...
    def _get_client(self):
        creds = Credentials(
            ...
            client_id=os.getenv("CLIENT_ID") or configure.CLIENT_ID,
            client_secret=os.getenv("CLIENT_SECRET") or configure.CLIENT_SECRET,
        )
    ...
    async def read_emails(self, max_results=10, query=''):
        """
        query examples:
        ...
        results = await asyncio.to_thread(
            self._client.users().messages().list(
                userId='me',
        ...
        for msg in messages:
            email_data = await asyncio.to_thread(
                self._client.users().messages().get(
                    userId='me',
                    id=msg['id'],
                    format='full'
                ).execute
    ...
    async def send_email(self, to, subject, body, cc=None, bcc=None, attachment_path=None):
        msg = MIMEMultipart()
        msg['To']      = to
        msg['Subject'] = subject
        if cc:  msg['Cc']  = cc
        ...
        result = await asyncio.to_thread(
            self._client.users().messages().send(
                userId='me',
                body={'raw': raw}
            ).execute
        )
