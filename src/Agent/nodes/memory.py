...
extract_chain = (get_extract_prompt() | llm.with_structured_output(extract_data, method="json_mode")).with_retry(stop_after_attempt=3)
...
async def memory(state: GmailState, config=None) -> dict:
    ...
    if user_uid:
        ...
        try:
            ...
            try:
                response = await extract_chain.ainvoke({"query": query, "existing_memory": stored_data})
                if response and response.should:
                    if response.memories.is_new and response.memories.content:
                        await store.aput(namespace, str(uuid.uuid4()), {"data": response.memories.content})
