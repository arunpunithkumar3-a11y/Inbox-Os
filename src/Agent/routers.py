...
from src.Agent.schemas import AgentRequest,ResumeRequest
from langgraph.types import Command
from langgraph.errors import GraphInterrupt
from src.auth.dependency import verify_token
from src.connect_gmail.service import GoogleService
from src.connect_gmail.utils import refresh_access_token
from src.db.main import get_session

logger = logging.getLogger(__name__)

Agent_router = APIRouter()
google_serv = GoogleService()
...
async def build_agent_instance(
    ...
    user_gmail_data = await google_serv.get_gmail_user_by_id(
        id=user_id,
        session=session
    )

    if not user_gmail_data:
        raise HTTPException(
    ...
    mcp_data = {
        "access_token": user_gmail_data.access_token,
        "refresh_token": user_gmail_data.refresh_token,
    }
...
async def get_chats(
    ...
    if not user_gmail_data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail={"message": "Gmail account not connected for this user"},
        )
    ...
    agent = GmailAgent(thread_id=thread_id, id=user_id, p_data=mcp_data)
    try:
